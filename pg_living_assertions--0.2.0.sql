...
DECLARE
    ...
    d text;
BEGIN
    SELECT a.check_sql INTO q FROM assertions a WHERE a.id = p_id;
    IF q IS NULL THEN
...
BEGIN
    ...
    t0 := clock_timestamp();
    SELECT e.state, e.detail INTO st, de FROM _evaluate(a.id) e;

    INSERT INTO checks (assertion, state, detail, duration_ms)
    VALUES (a.id, st, de,
...
BEGIN
    IF p_supersedes IS NOT NULL THEN
        SELECT id INTO old_id FROM assertions
    ...
    INSERT INTO assertions (name, claim, check_sql, supersedes, why_changed)
